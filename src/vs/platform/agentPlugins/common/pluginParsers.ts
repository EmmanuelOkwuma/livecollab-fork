 ...
 * using LiveCollab's `${env:VAR}` colon-delimited syntax.
 */
const BARE_ENV_VAR_RE = /\$\{(?![A-Za-z]+:)([A-Z_][A-Z0-9_]*)\}/g;

/**
 * Converts bare `${VAR}` environment-variable references to LiveCollab `${env:VAR}` syntax.
 */
 ...
 * `.instructions.md` for compatibility with LiveCollab-discovered instructions
