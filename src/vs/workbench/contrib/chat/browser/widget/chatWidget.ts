...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	render(parent: HTMLElement): void {
		...
		// max-width margins in the classic LiveCollab chat view) to the chat list.
	...
	private _getGenerateInstructionsMessage(): IMarkdownString {
		...
		if (!this._instructionFilesCheckPromise) {
			...
			// Use LiveCollab's idiomatic pattern for disposal-safe promise callbacks
		...
		if (this.isLockedToCodingAgent) {
			...
			const message = providerMessage
				...
				: (this._lockedAgent?.prefix === '@copilot '
					? new MarkdownString(localize('copilotCodingAgentMessage', "This chat session will be forwarded to the {0} [coding agent]({1}) where work is completed in the background. ", this._lockedAgent.prefix, '') + DISCLAIMER, { isTrusted: true })
