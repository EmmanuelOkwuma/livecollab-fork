...
configurationRegistry.registerConfiguration({
	...
	properties: {
		...
		[ChatConfiguration.EditorAssociations]: {
			type: 'object',
			markdownDescription: nls.localize('chat.editorAssociations', "Configure glob patterns to editors for opening files from chat (for example `\"*.md\": \"vscode.markdown.preview.editor\"`)."),
			additionalProperties: {
				type: 'string'
			},
			default: {
		...
		[mcpAccessConfig]: {
			...
			enumDescriptions: [
				...
				nls.localize('chat.mcp.access.registry', "Allows access to MCP servers installed from the registry that LiveCollab is connected to."),
			...
			policy: {
				...
				localization: {
					...
					enumDescriptions: [
						...
						{
							key: 'chat.mcp.access.registry', value: nls.localize('chat.mcp.access.registry', "Allows access to MCP servers installed from the registry that LiveCollab is connected to."),
						},
						{
		...
		[ChatConfiguration.ExtraMarketplaces]: {
			...
			// JSON Schema's `type` keyword (equivalent to `type: 'string'`) to trigger LiveCollab's
		...
		[PromptsConfig.PROMPT_FILES_SUGGEST_KEY]: {
			...
			markdownDescription: nls.localize(
				...
				"Configure which prompt files to recommend in the chat welcome view. Each key is a prompt file name, and the value can be `true` to always recommend, `false` to never recommend, or a when clause expression like `resourceExtname == .js` or `resourceLangId == markdown`.",
			),
			default: {},
			additionalProperties: {
				oneOf: [
		...
		[ChatConfiguration.TitleBarSignInEnabled]: {
			...
			default: false, // LiveCollab: hidden by default
		},
