...
const taskDefinitionSchema: IJSONSchema = {
	...
	properties: {
		...
		required: {
			type: 'array',
			markdownDescription: nls.localize('TaskDefinition.required', 'The names of the properties from the `properties` object that must be provided for a task of this type to be considered a match. Used by LiveCollab to associate a `tasks.json` entry with a registered task provider.'),
			items: {
				type: 'string'
			}
		},
		...
		when: {
			type: 'string',
			markdownDescription: nls.localize('TaskDefinition.when', 'Condition which must be true to enable this type of task. Consider using `shellExecutionSupported`, `processExecutionSupported`, and `customExecutionSupported` as appropriate for this task definition. See the API documentation for more information.'),
			default: ''
		}
	}
};
