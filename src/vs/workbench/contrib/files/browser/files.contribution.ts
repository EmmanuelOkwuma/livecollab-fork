...
const hotExitConfiguration: IConfigurationPropertySchema = isNative ?
	{
		...
		'default': HotExitConfiguration.ON_EXIT, // LiveCollab: reverted back to stock default - PHASE3_YJS_DESIGN.md section 17. The section 16 fix (setting this to OFF) was WRONG despite sound-seeming reasoning: a real, confirmed A/B test showed folders stopped opening entirely, on both machines, with hot exit off, and opened again immediately once reverted. VS Code's folder-opening mechanism has some real, unexpected dependency on this service being active. The original section 16 problem (stale room folders accumulating in the hot-exit backup file) is real and still unsolved - needs a more surgical fix that doesn't disable the whole mechanism, e.g. explicitly clearing this room's own backup entry on room leave, not turning hot exit off globally.
