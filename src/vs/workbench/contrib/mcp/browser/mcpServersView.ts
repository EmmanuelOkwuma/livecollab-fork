...
export class McpServersListView extends AbstractExtensionsListView<IWorkbenchMcpServer> {
	...
	private createWelcomeContent(welcomeContainer: HTMLElement): void {
		...
		const markdownResult = this._register(this.markdownRendererService.render(
			new MarkdownString(
				localize('mcp.welcome.descriptionWithLink', "Browse and install Model Context Protocol (MCP) servers directly from LiveCollab to extend agent mode with extra tools for connecting to databases, invoking APIs and performing specialized tasks."),
