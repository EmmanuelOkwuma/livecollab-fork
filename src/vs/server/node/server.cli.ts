...
/*
 * Implements a standalone CLI app that opens LiveCollab from a remote terminal.
 ...
 *  - In external terminals for WSL this calls LiveCollab on the Windows side.
 *    The LiveCollab desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
function fatal(message: string, err: unknown): void {
	console.error('Unable to connect to LiveCollab server: ' + message);
	console.error(err);
	process.exit(1);
}
